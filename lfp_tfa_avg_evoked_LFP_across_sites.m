...
for t = 1:length(lfp_tfa_cfg.compare.targets)
        ...
        if ~isempty(sites_avg(t).condition(cn).hs_tuned_evoked)
            if isfield(sites_avg(t).condition(cn).hs_tuned_evoked,...
                ...
                result_file = fullfile(results_fldr, ...
                    ['LFP_Evoked_' lfp_tfa_cfg.compare.targets{t} '_' lfp_tfa_cfg.conditions(cn).label ]);
    ...
    for trial_type = 1:2
        plottitle = [lfp_tfa_cfg.compare.targets{t},...
            '(ref_' lfp_tfa_cfg.ref_hemisphere, ')_combined_'];
        if trial_type == 1
        ...
        result_file = fullfile(results_fldr,['LFP_Evoked_' lfp_tfa_cfg.compare.targets{t} ...
            '_combined_' trial_title ]);
        
        lfp_tfa_plot_evoked_lfp_combined(sites_avg(t).condition, lfp_tfa_cfg, ...
