function lfp_tfa_plot_evoked_lfp_combined_accross_sessions( evoked_lfp, lfp_tfa_cfg, plottitle, results_file,trial_type )
...
for cnd = 1:length(cond_to_plot)
    ...
    for hs = 1:size(evoked_lfp(cond_to_plot(cnd)).avg_across_sessions, 2)
        if ~isempty([evoked_lfp(cond_to_plot(cnd)).avg_across_sessions(:,hs).lfp])% &&  ~isempty([evoked_lfp(:,hs).std])
            % concatenate states
            ...
            state_info = struct();
            for st = 1:size(evoked_lfp(cond_to_plot(cnd)).avg_across_sessions, 1)
                
                state_info(cnd,st).onset_s = find(...
                    evoked_lfp(cond_to_plot(cnd)).avg_across_sessions(st, hs).time <= 0, 1, 'last');
                state_info(cnd,st).onset_t = 0;
                state_info(cnd,st).start_s = 1;
                state_info(cnd,st).start_t =  evoked_lfp(cond_to_plot(cnd)).avg_across_sessions(st, hs).time(1);
                state_info(cnd,st).finish_s = length( evoked_lfp(cond_to_plot(cnd)).avg_across_sessions(st, hs).time);
                state_info(cnd,st).finish_t =  evoked_lfp(cond_to_plot(cnd)).avg_across_sessions(st, hs).time(end);
                ...
                [state_mean, state_error] = ...
                    lfp_tfa_compute_statistics(evoked_lfp(cond_to_plot(cnd)).avg_across_sessions(st, hs).lfp, lfp_tfa_cfg.error_measure);
                
                concat_states_lfp(cnd).lfp = [concat_states_lfp(cnd).lfp, ...
                    evoked_lfp(cond_to_plot(cnd)).avg_across_sessions(st, hs).lfp, ...
                    nan(size(evoked_lfp(cond_to_plot(cnd)).avg_across_sessions(st, hs).lfp,1), noffset)];
                ...
                concat_states_lfp(cnd).time = [concat_states_lfp(cnd).time, ...
                    evoked_lfp(cond_to_plot(cnd)).avg_across_sessions(st, hs).time, nan(1, noffset)];
                concat_states_lfp(cnd).label = evoked_lfp(cond_to_plot(cnd)).avg_across_sessions(st, hs).hs_label;
        ...
        if cnd == 1
            ...
            for so = state_onsets
                ...
                if isfield(evoked_lfp(cond_to_plot(cnd)).avg_across_sessions(state_onsets == so, hs), 'state_name') && ...
                        ~isempty(evoked_lfp(cond_to_plot(cnd)).avg_across_sessions(state_onsets == so, hs).state_name)
                    state_name = evoked_lfp(cond_to_plot(cnd)).avg_across_sessions(state_onsets == so, hs).state_name;
                    ypos = ylim;
