function sites_avg = lfp_avg_across_sites(Input, lfp_tfa_cfg, varargin)
...
%%%%%%%%%%%%%%%%%%%%%%%%%[DAG mfile header version 1]%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for t = 1:length(lfp_tfa_cfg.compare.targets)
        ...
        for cn = 1:length(lfp_tfa_cfg.conditions)
            ...
            % label
            for ep = 1:size(Input.session(1).sites(1).condition(cn).tuned, 1)
                for hs = 1:size(Input.session(1).sites(1).condition(cn).tuned, 2)
                    sites_avg(t).condition(cn).tuned(ep, hs).nsites = 0;
                    sites_avg(t).condition(cn).tuned(ep, hs).psd = [];
                end
            end
            nsites = 0;
            for i = 1:length(Input.session)  
                for j = 1:length(Input.session(i).sites)
                    ...
                    if ~isempty(Input.session(i).sites(j).condition(cn).tuned) && ... 
                        ...
                        nsites = nsites + 1;   
                        for ep = 1:size(Input.session(i).sites(j).condition(cn).tuned, 1)
                            for hs = 1:size(Input.session(i).sites(j).condition(cn).tuned, 2)
                                if isfield(Input.session(i).sites(j).condition(cn).tuned(ep, hs), 'mean') ...
                                        && ~isempty(Input.session(i).sites(j).condition(cn).tuned(ep, hs).mean)
                                    sites_avg(t).condition(cn).tuned(ep, hs).nsites = ...
                                        sites_avg(t).condition(cn).tuned(ep, hs).nsites + 1;
                                    if sites_avg(t).condition(cn).tuned(ep, hs).nsites == 1%~isfield(sessions_avg.cond_based_tfs(cn).tfs_across_sessions, 'powspctrm')
                                        sites_avg(t).condition(cn).tuned(ep,hs).freq ...
                                        = Input.session(i).sites(j).condition(cn).tuned(ep, hs).freq;
                                        sites_avg(t).condition(cn).tuned(ep,hs).hs_label ...
                                            = Input.session(i).sites(j).condition(cn).tuned(ep, hs).hs_label;
                                        sites_avg(t).condition(cn).tuned(ep,hs).epoch_name ...
                                            = Input.session(i).sites(j).condition(cn).tuned(ep, hs).epoch_name;
                                        sites_avg(t).condition(cn).tuned(ep,hs).psd ...
                                            = Input.session(i).sites(j).condition(cn).tuned(ep, hs).mean;
                                    else
                                        nfreqbins = length(sites_avg(t).condition(cn).tuned(ep, hs).freq);
                                        % average same number of time bins
                                        if nfreqbins > length(Input.session(i).sites(j).condition(cn).tuned(ep, hs).freq)
                                            nfreqbins = length(Input.session(i).sites(j).condition(cn).tuned(ep, hs).freq);
                                        end
                                        sites_avg(t).condition(cn).tuned(ep,hs).freq = ...
                                            Input.session(i).sites(j).condition(cn).tuned(ep, hs).freq(1:nfreqbins);
                                        sites_avg(t).condition(cn).tuned(ep,hs).psd ...
                                            = [Input.session(i).sites(j).condition(cn).tuned(ep, hs).mean(1:nfreqbins); ...
                                            sites_avg(t).condition(cn).tuned(ep,hs).psd(:,1:nfreqbins)];                              
                                    end
                                end
                            end
                        end
            ...
            % compute average
            for ep = 1:size(sites_avg(t).condition(cn).tuned, 1)
                for hs = 1:size(sites_avg(t).condition(cn).tuned, 2)
                    if isfield(sites_avg(t).condition(cn).tuned(ep,hs), 'psd')
                        sites_avg(t).condition(cn).tuned(ep,hs).dimord = 'nsites_freq';                                
                        [sites_avg(t).condition(cn).tuned(ep,hs).mean, ...
                            sites_avg(t).condition(cn).tuned(ep,hs).error] = ...
                            (lfp_tfa_compute_statistics(...
                            sites_avg(t).condition(cn).tuned(ep,hs).psd, lfp_tfa_cfg.error_measure));
                    end
                end
            end
            ...
            if ~isempty(sites_avg(t).condition(cn).tuned)
                if isfield(sites_avg(t).condition(cn).tuned,... 
                    ...
                    lfp_tfa_plot_hs_tuned_psd_2(sites_avg(t).condition(cn).tuned, ...
