function lfp_tfa_plot_hs_tuned_tfr_band_average( avg_tfr, lfp_tfa_cfg, plottitle, results_file, varargin )
...
for nband = 1:length(fieldnames(lfp_tfa_cfg.band))
    ...
    for hs = 1:size(avg_tfr, 2)
        ...
        % check if no trials exist for this condition and HS
        if isfield(cat(2, avg_tfr(:, hs).freq), band_labels{nband})
            freq =  cat(2, avg_tfr(:, hs).freq);
        else
            continue;
        end
        if ~isempty(cat(3, freq.(band_labels{nband})))
            ...
            for st = 1:size(avg_tfr, 1)
                ...
                concat_states_tfs.powspctrm = cat(3, concat_states_tfs.powspctrm, ...
                    avg_tfr(st, hs).freq.(band_labels{nband}), ...
                    nan(size(avg_tfr(st, hs).freq.(band_labels{nband}), 1), 1, 100/25));
                concat_states_tfs.state_time = [concat_states_tfs.state_time, ...
                    avg_tfr(st, hs).freq.time, nan(1, 100/25)];
                
                
                state_lfp_powspctrm = nanmean(avg_tfr(st, hs).freq.(band_labels{nband}),1);
                
               stderr = nanstd(avg_tfr(st, hs).freq.(band_labels{nband}),1, 1)...
                   / sqrt(size(avg_tfr(st, hs).freq.(band_labels{nband}),1));
...
%             end
    ...
    % plot title
    plottitle_tmp = [];
     plottitle_tmp = [plottitle ' ' band_labels{nband}];
