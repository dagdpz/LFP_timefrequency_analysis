function [sites_tfr] = lfp_tfa_plot_site_average_tfr( site_lfp, site_conditions, lfp_tfa_cfg ) 
...
% lfp_tfa_plot_hs_tuned_tfr_multiple_img
    ...
    session_tfs = struct();
    % session name
    session_tfs.session = site_lfp(1).session;
...
%     site_conditions = lfp_tfa_compare_conditions(lfp_tfa_cfg, perturbation_groups);
    ...
    for i = 1:length(site_lfp)
        ...
        sites_tfr(i).site_ID = site_lfp(i).site_ID;
        sites_tfr(i).session = site_lfp(i).session;
        sites_tfr(i).target = site_lfp(i).target;
...
%         session_avg(t).session = site_lfp(1).session;
