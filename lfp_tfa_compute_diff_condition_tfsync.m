                    ...
                    for hs = 1:size(postinj_tfr.hs_tuned_sync, 2)
                        for st = 1:size(postinj_tfr.hs_tuned_sync, 1)
                            ...
                            if isfield(preinj_sync.hs_tuned_sync(st, hs).ppc, 'ppcspctrm') && ...
                                ...
                                % difference in cross spectrum absolute
                                % value and angle
                                diff_sync.difference(dcn).hs_tuned_sync(st, hs).csd.crsspctrm_abs_mean_norm = ...
                                    postinj_tfr.hs_tuned_sync(st, hs).csd.crsspctrm_abs_mean_norm(:,:,1:ntimebins) - ...
                                    preinj_sync.hs_tuned_sync(st, hs).csd.crsspctrm_abs_mean_norm(:,:,1:ntimebins);
                                
                                diff_sync.difference(dcn).hs_tuned_sync(st, hs).csd.crsspctrm_ang_mean = ...
                                    postinj_tfr.hs_tuned_sync(st, hs).csd.crsspctrm_ang_mean(:,:,1:ntimebins) - ...
                                    preinj_sync.hs_tuned_sync(st, hs).csd.crsspctrm_ang_mean(:,:,1:ntimebins);
