function sessions_avg = lfp_tfa_avg_evoked_LFP_per_session(lfp_evoked, lfp_tfa_cfg)

    % results folder
    results_fldr = fullfile(lfp_tfa_cfg.root_results_fldr, 'Average_Evoked');
    ...
    for cn = 1:length(lfp_evoked.session(1).condition)
        ...
        for i = 1:length(lfp_evoked.session)  
            if ~isempty(lfp_evoked.session(i).condition(cn).evoked_lfp_session) && ... 
                isfield(lfp_evoked.session(i).condition(cn).evoked_lfp_session, 'mean')
                nsessions = nsessions + 1;   
                %sessions_avg.cond_based_tfs(cn).tfs_across_sessions = struct();
                for st = 1:size(lfp_evoked.session(i).condition(cn).evoked_lfp_session, 1)
                    for hs = 1:size(lfp_evoked.session(i).condition(cn).evoked_lfp_session, 2)
                        if isfield(lfp_evoked.session(i).condition(cn).evoked_lfp_session(st, hs), 'mean') ...
                                && ~isempty(lfp_evoked.session(i).condition(cn).evoked_lfp_session(st, hs).mean)
                            if nsessions == 1%~isfield(sessions_avg.condition(cn).avg_across_sessions, 'mean')
                                ...
                                = lfp_evoked.session(i).condition(cn).evoked_lfp_session(st, hs).time;
                                sessions_avg.condition(cn).avg_across_sessions(st,hs).hs_label ...
                                    = lfp_evoked.session(i).condition(cn).evoked_lfp_session(st, hs).hs_label;
                                sessions_avg.condition(cn).avg_across_sessions(st,hs).state ...
                                    = lfp_evoked.session(i).condition(cn).evoked_lfp_session(st, hs).state;
                                sessions_avg.condition(cn).avg_across_sessions(st,hs).mean ...
                                    = lfp_evoked.session(i).condition(cn).evoked_lfp_session(st, hs).mean;
                                sessions_avg.condition(cn).avg_across_sessions(st,hs).std ...
                                    = lfp_evoked.session(i).condition(cn).evoked_lfp_session(st, hs).std;                                
                                if isfield(lfp_evoked.session(i).condition(cn).evoked_lfp_session(st, hs), 'nsites')
                                    sessions_avg.condition(cn).avg_across_sessions(st,hs).nsites ...
                                        = lfp_evoked.session(i).condition(cn).evoked_lfp_session(st, hs).nsites;
                                end
                            else
                                ntimebins = length(sessions_avg.condition(cn).avg_across_sessions(st, hs).time);
                                % average same number of time bins
                                if ntimebins > length(lfp_evoked.session(i).condition(cn).evoked_lfp_session(st, hs).time)
                                    ntimebins = length(lfp_evoked.session(i).condition(cn).evoked_lfp_session(st, hs).time);
                                end
                                sessions_avg.condition(cn).avg_across_sessions(st,hs).mean ...
                                    = (lfp_evoked.session(i).condition(cn).evoked_lfp_session(st, hs).mean(1:ntimebins)) + ...
                                    sessions_avg.condition(cn).avg_across_sessions(st,hs).mean(1:ntimebins);
                                sessions_avg.condition(cn).avg_across_sessions(st,hs).std ...
                                    = (lfp_evoked.session(i).condition(cn).evoked_lfp_session(st, hs).std(1:ntimebins)) + ...
        ...
        if ~isempty(sessions_avg.condition(cn).avg_across_sessions)
            if isfield(sessions_avg.condition(cn).avg_across_sessions,... 
                ...
                result_file = fullfile(results_fldr, ...
                                ['Avg_Evoked_LFP_' lfp_evoked.session(1).condition(cn).label '.png']);
