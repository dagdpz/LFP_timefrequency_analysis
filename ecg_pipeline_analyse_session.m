function [ session_avg ] = ecg_pipeline_analyse_session( session_ecg, session_info, analyse_states, lfp_tfa_cfg ) 
...
% lfp_tfa_compare_conditions, lfp_tfa_plot_evoked_lfp
    ...
    session_avg = struct();
    session_avg.session = session_ecg(1).session;
    ...
    for i = 1:length(session_ecg) 
        ...
        for cn = 1:length(site_conditions)
            ...
            session_avg(i).condition(cn).label = site_conditions(cn).label;
            session_avg(i).condition(cn).cfg_condition = site_conditions(cn);
            session_avg(i).condition(cn).ntrials = zeros(1,length(hs_labels));  
            session_avg(i).condition(cn).Rpeak_evoked_ECG = struct(); 
            session_avg(i).condition(cn).Rpeak_evoked_LFP = struct();
            session_avg(i).condition(cn).Rpeak_evoked_state_proba = struct();
            session_avg(i).condition(cn).state_evoked_ECG = struct();
            session_avg(i).condition(cn).state_evoked_R2Rt = struct();
            session_avg(i).condition(cn).state_evoked_Rpeak_proba = struct();
            ...
            for hs = 1:length(hs_labels)
                ...
                session_avg(i).condition(cn).ntrials(hs) = sum(cond_trials);

                fprintf('Condition %s - %s\n', site_conditions(cn).label, hs_labels{hs});
                fprintf('Total number of trials %g\n', sum(cond_trials));

                session_avg(i).condition(cn).noisytrials(hs) = ...
                ...
                for st = 1:size(analyse_states, 1)                    
                    ...
                    % Rpeak evoked state onset probability
                    session_avg(i).condition(cn).Rpeak_evoked_state_proba(st, hs) = ...
                        ecg_pipeline_compute_Rpeak_evoked_state_proba(...
            ...
            % plots
            % Rpeak evoked state onset probability
            if ~isempty(fieldnames(session_avg(i).condition(cn).Rpeak_evoked_state_proba))
                plottitle = ['Session: ', session_avg(i).session ...
                    site_conditions(cn).label '), '];
                
                result_file = fullfile(site_results_folder, ...
                    ['Rpeak_Evoked_state_proba_' session_avg(i).session '_' site_conditions(cn).label '.png']);

                ecg_pipeline_plot_Rpeak_evoked_state_proba (...
                    session_avg(i).condition(cn).Rpeak_evoked_state_proba, lfp_tfa_cfg, ...
...
%         end
        ...
        site_evoked_ecg = session_avg(i);
        % save mat file for site
        save(fullfile(site_results_folder, ['Rpeak_Evoked_state_onsets_' session_avg(i).session '.mat']), 'site_evoked_ecg');
        % save to a mother struct
        session_avg = site_evoked_ecg;
    end
