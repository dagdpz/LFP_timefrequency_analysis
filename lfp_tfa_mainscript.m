...
% Read the list of files containing LFP data to analyse
file_list = lfp_tfa_cfg.file_list;
% 3rd element is the path to the mat file containing LFP data
...
try
    ...
    for i = 1:length(lfp_datafiles)
        ...
        % read LFP data for each site and each trial and calculate the trial-wise time
    ...
    for i = 1:length(session_proc_lfp)
        ...
        % absolute path of file containing LFP data for this session
        lfp_tfa_cfg.data_filepath = lfp_datafiles{i};
        % Calculate and plot the site-wise and session average TFR, evoked response and power spectral density for 
...
if length(session_proc_lfp) > 1
    ...
    lfp_tfr.sessions_avg = lfp_tfa_avg_tfr_across_sessions(lfp_tfr, lfp_tfa_cfg);
    lfp_evoked.sessions_avg = lfp_tfa_avg_evoked_LFP_across_sessions(lfp_evoked, lfp_tfa_cfg);
    lfp_pow.sessions_avg = lfp_tfa_avg_pow_across_sessions(lfp_pow, lfp_tfa_cfg);
    end
    ...
    if any(strcmp(lfp_tfa_cfg.compute_avg_across, 'sites'))
        ...
        % power spectrum
        lfp_tfr.sites_avg = lfp_tfa_avg_tfr_across_sites(lfp_tfr, lfp_tfa_cfg);
